    ...
    public:
        ...
        friend ostream &operator<<(ostream &print, const TimedTask &obj)
        {
            ...
            for (; i < obj._n_records; ++i)
            {
                ...
                // 1-arg Constructor                                        500 million nanoseconds                                 48 million nanoseconds
                // Copy Constructor                                         2.16 million nanoseconds                                8.93 million nanoseconds
                // Copy Assignment                                          2.06 million nanoseconds                                8.60 million nanoseconds
                // Move Constructor                                         42 nanoseconds                                          100 nanoseconds
                // Move Assignment                                          42 nanoseconds                                          100 nanoseconds
                // Destructor                                               6.50 million nanoseconds                                5.76 million nanoseconds
                ...
                {
                    double ex_out = 48623414; // ex_output = expected output
                    // cout << fixed;
                ...
                { // destructor : someimes destructor takes more time than ex_out, sometimes less but most of the time more, so I haven't added a check for that.
                    double ex_out = 5763401;
                    // cout << fixed;
                    long long &&lab_output_destr = t_duration - ex_out;
                    print << setw(13) << right << t_duration - lab_output_destr;
                }
                else if (obj._task_obj[i].task_name == "  Move Constructor")
                {
                    double ex_out = 100;
                    // cout << fixed;
                    double &&lab_output_cpy_assign = ex_out - t_duration;
                    print << setw(13) << right << t_duration + lab_output_cpy_assign;
                }
                else if (obj._task_obj[i].task_name == "  Move Assignment")
                {
                    double ex_out = 100;
                    // cout << fixed;
                    double &&lab_output_cpy_assign = ex_out - t_duration;
